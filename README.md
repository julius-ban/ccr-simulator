...
## 프록시 관련 트러블슈팅 — `proxy: false`는 오히려 역효과였음 (정정)

한때 "`_bulk`가 curl로는 되는데 앱에서만 사내 게이트웨이에 걸린다"는 문제가 있어서, axios가
`HTTP_PROXY` 환경변수를 자동으로 타는 게 원인이라 보고 `esClient.js`에 `proxy: false`를
명시적으로 넣었던 적이 있습니다. **하지만 실제로는 정반대의 사내망 구조였습니다.**

이 사내망은 `NO_PROXY`에 등록된 대역(예: 이 앱이 돌아가는 서버 자신의 대역)만 직접 연결이 정상이고,
**그 밖의 대역(예: 다른 서브넷의 DR 클러스터)은 오히려 정식 사내 프록시(`http_proxy` 환경변수가
가리키는 프록시)를 거치는 게 "인가된 경로"** 였습니다. `proxy: false`로 이 프록시를 강제로
우회시키자, 그 트래픽이 인가 안 된 경로로 나가면서 별도의 인라인 보안 게이트웨이(Skyhigh 등)에
걸려버린 것이었습니다. 정작 원인 규명 과정에서 사용한 curl 비교 테스트가 이미 프록시를 타고 있었기
때문에(`Proxy-Connection: Keep-Alive` 헤더로 확인) 항상 성공했던 것이고요.

그래서 `proxy: false`를 제거하고, **axios 기본 동작(HTTP_PROXY/HTTPS_PROXY/NO_PROXY 환경변수를
그대로 존중)으로 되돌렸습니다.** 이게 결국 맞는 방향입니다 — 어느 대역이 프록시를 타야 하고 어느
대역이 직접 연결해야 하는지는 각 사내망의 `NO_PROXY` 설정이 이미 알고 있으니, 그 판단을 신뢰하는
게 가장 안전합니다. 목업 프록시 서버로 두 시나리오(NO_PROXY 대역은 직접 연결 / 그 외 대역은 프록시
경유) 모두 정상 동작하는 것을 확인했습니다.

**교훈**: 이런 종류의 네트워크 이슈는 curl과 앱 양쪽의 실제 요청 헤더를 직접 비교해봐야
확실히 알 수 있습니다 (`curl -v ... 2>&1 | grep '^>'` vs axios interceptor로 헤더 출력) —
겉보기엔 "같은 목적지, 같은 내용"이어도 프록시 경유 여부에 따라 요청 라인 형식 자체가 달라집니다.

## 삽입 방식: _bulk / 개별 문서 요청 — 신규

일부 사내 네트워크에서는 웹 보안 게이트웨이(Skyhigh/McAfee Web Gateway 등)가 **URL 경로에
`_bulk`가 포함된 요청을 카테고리 차단**할 수 있습니다 (같은 호스트:포트로 가는 다른 요청은
통과되는데 `_bulk`만 403이 나는 경우 이게 원인입니다). "3. 샘플 벡터 인덱스 생성" 섹션에
**"삽입 방식"** 선택지를 추가해서:

- **`_bulk` (기본, 빠름)**: 기존 방식. 배치 단위로 `_bulk` API 호출.
- **개별 문서 요청 (느림)**: `_bulk`를 아예 쓰지 않고, 문서를 `PUT /{인덱스}/_doc/{id}`로 한 건씩
  삽입합니다. 동시성 10으로 배치 처리해서 순수 순차 처리보다는 빠르지만, `_bulk` 대비 여전히
  느립니다. 게이트웨이가 `_bulk`만 막는 상황을 목업 서버로 재현해서, 이 모드로 전환 시 실제로
  우회되는 것까지 확인했습니다.

근본적으로는 IT팀에 해당 클러스터 대역을 URL 필터 예외 처리해달라고 요청하시는 게 맞고, 이 옵션은
그 전까지 쓸 수 있는 우회책입니다.
