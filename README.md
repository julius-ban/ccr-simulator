...
### 🧭 가이드 모드 (기본 켜짐)
헤더 토글이 기본적으로 켜져 있어서, 페이지를 열면 바로 지금 눌러야 할 다음 버튼에 주황색
pulsing 테두리가 생기고, 아직 순서가 안 된 이후 버튼들은 흐리게 표시됩니다. 클러스터 등록 →
샘플 인덱스 생성 → CCR 연동 4단계 → 모니터링 → Failover → Failback 6단계까지, 총 13개 스텝을
순서대로 추적합니다 (`GUIDE_STEPS` 배열, `markGuideStep()` 호출로 각 버튼 클릭 시 다음 단계로
넘어감). 필요 없으면 토글을 꺼서 자유롭게 아무 버튼이나 누를 수 있습니다.
