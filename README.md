...
### CCR 인증 방식
- **API Key 인증 (기본)**: 지금까지와 동일한 방식입니다. ① API Key 발급 → ② keystore 등록 →
  ③ Remote 등록(Proxy 모드 고정, remote cluster server 포트 사용) 순서로 진행합니다.
- **TLS 인증서 기반 인증 (구 방식)**: API Key 발급/keystore 단계 버튼이 아예 숨겨지고, 대신
